...
def eval_not_data_cell_rule(
    ...
    if rule == "First_FW_Symbol_disagrees":
        if (
            len(value_pattern_summary) != 0
            and cand_pattern[0][0] != value_pattern_summary[0][0]
        ):
            rule_fired = True

    elif rule == "First_BW_Symbol_disagrees":
        if (
            len(value_pattern_BW_summary) != 0
            and cand_pattern[-1][0] != value_pattern_BW_summary[0][0]
        ):
        ...
        if (
            cand_length > 0
            and cand_length != summary_min_length
            and summary_min_length == summary_max_length
        ):
        ...
        if (
            ...
            and cand_length > 0
            and cand_length >= 1.1 * summary_max_length
        ):
            rule_fired = True
        ...
        if (
            summary_max_length > 0
            and cand_length > 0
            and cand_length >= 1.3 * summary_max_length
        ):
            rule_fired = True
        ...
        if (
            summary_max_length > 0
            and cand_length > 0
            and cand_length >= 1.5 * summary_max_length
        ):
            rule_fired = True
        ...
        if (
            summary_max_length > 0
            and cand_length > 0
            and cand_length >= 1.6 * summary_max_length
        ):
            rule_fired = True
        ...
        if (
            summary_max_length > 0
            and cand_length > 0
            and cand_length >= 1.7 * summary_max_length
        ):
            rule_fired = True
        ...
        if (
            summary_max_length > 0
            and cand_length > 0
            and cand_length >= 1.8 * summary_max_length
        ):
            rule_fired = True
        ...
        if (
            summary_max_length > 0
            and cand_length > 0
            and cand_length >= 1.9 * summary_max_length
        ):
            rule_fired = True
        ...
        if (
            summary_strength > 1
            and value_chain_consistent == True
            and pat.symbol_chain_disagrees(value_symbol_chain, cand_symbol_chain)
        ):
        ...
        if (
            line_index + 1 in line_agreements.keys()
            and line_index in line_agreements.keys()
        ):
            ...
            if columnindex in next_line_agreements.keys():
                ...
                if (
                    line_agreements[line_index][columnindex]["null_equivalent"] == False
                    and rule in value_below_agreements
                    and columnindex in line_agreements[line_index].keys()
                    and rule
                    not in line_agreements[line_index][columnindex]["agreements"]
                ):
                    rule_fired = True
...
def eval_data_cell_rule(
    ...
    if candidate == None or str(candidate).strip().lower() in null_equivalent:
        ...
        if (
            consistent_symbol_chain
            and len(value_symbol_summary) == 1
            and value_symbol_summary[0] == "A"
            and case_summary in ["ALL_CAPS", "ALL_LOW", "TITLE"]
        ):
            rule_fired = True

    elif rule == "CONSISTENT_CHAR_LENGTH":
        rule_fired = False
        if consistent_symbol_chain == True and str(candidate).strip().lower() not in [
            "",
            " ",
            "nan",
            "None",
        ...
        if (
            len(columnvalues) > 2
            and sum([partof_multiword_value_repeats[t] for t in candidate_tokens]) == 1
        ):
            rule_fired = True
        ...
        if (
            len(columnvalues) > 2
            and sum([partof_multiword_value_repeats[t] for t in candidate_tokens]) >= 2
        ):
            rule_fired = True
        ...
        if (
            ...
            and len(all_values_summary) == 1
            and all_values_summary[0][0] == "D"
            and all_values_summary[0][1] > 0
        ):
            rule_fired = True
        ...
        if (
            all_patterns_numeric
        ):  # pat.numeric_patterns(column_symbols, column_patterns):
            rule_fired = True
    elif rule == "FW_THREE_OR_MORE_NO_SPACE":
        # "Rule_6 three or above symbols in FW summary that do not contain a  Space"
        rule_fired = False
        if len(all_values_summary) >= 3 and "S" not in [
            x[0] for x in all_values_summary
        ]:
            rule_fired = True
    elif rule == "BW_THREE_OR_MORE_NO_SPACE":
        # "Rule_7 three or above symbols in BW summary that do not contain a  Space"
        rule_fired = False
        if len(pattern_BW_summary) >= 3 and "S" not in [
            x[0] for x in pattern_BW_summary
        ]:
            rule_fired = True
    elif rule == "CONSISTENT_SS_NO_SPACE":
        ...
        if (
            consistent_symbol_set
            and len(value_symbol_summary) >= 2
            and "S" not in value_symbol_summary
            and "_" not in value_symbol_summary
        ):
            rule_fired = True
        ...
        rule_fired = False
        if len(all_values_summary) >= 2 and "S" not in [
            x[0] for x in all_values_summary
        ]:
            rule_fired = True
        ...
        rule_fired = False
        if len(pattern_BW_summary) >= 2 and "S" not in [
            x[0] for x in pattern_BW_summary
        ]:
            rule_fired = True
        ...
        rule_fired = False
        if len(all_values_summary) >= 2 and "S" not in [
            x[0] for x in all_values_summary[0:2]
        ]:
            rule_fired = True
        ...
        rule_fired = False
        if len(pattern_BW_summary) >= 2 and "S" not in [
            x[0] for x in pattern_BW_summary[0:2]
        ]:
            rule_fired = True
        ...
        rule_fired = False
        if (
            len(all_values_summary) == 1
            and all_values_summary[0][0] == "D"
            and all_values_summary[0][1] >= 5
        ):
            rule_fired = True
        ...
        rule_fired = False
        if (
            len(pattern_BW_summary) == 1
            and pattern_BW_summary[0][0] == "D"
            and pattern_BW_summary[0][1] >= 5
        ):
            rule_fired = True
...
def eval_numeric_pattern(train):
    ...
    if (
        "D" in symbolset
        and symbolset.issubset(set(["D", ",", ".", "-", "S"]))
        and (
            (
                [x[0] for x in train].count("-") == 1
                and len(train) > 0
                and train[0][0] == "-"
            )
            or ([x[0] for x in train].count("-") == 0)
        )
    ):
        return True
    else:
...
class TableSignatures:
    def __init__(self, dataframe=pd.DataFrame(), outlier_sensitive=False):
        if dataframe.empty == False:
            ...
            train = normalized_values.applymap(
                lambda cell: generate_train(cell, outlier_sensitive)
            )
            ...
            bw_train_normalized_numbers = train_normalized_numbers.applymap(
                lambda cell: list(reversed(cell))
            )
            symbolset_normalized_numbers = symbolset.applymap(
                symbolset_normalize_numbers
            )
            ...
            is_aggregate = tokens.applymap(
                lambda cell: (
                    len(cell) > 0 and not set(cell).isdisjoint(pat.aggregation_tokens)
                )
            )
            is_null_equivalent = normalized_values.applymap(
                lambda cell: (cell.lower() in pat.null_equivalent_values)
            )
    ...
    def reverse_slice(self, top, bottom):
        ...
        slice.all_column_character_lengths = self.all_column_character_lengths[
            top : bottom + 1
        ][::-1]
        slice.all_column_cases = self.all_column_cases[top : bottom + 1][::-1]
        slice.all_column_tokens = self.all_column_tokens[top : bottom + 1][::-1]
        slice.all_column_token_lengths = self.all_column_token_lengths[
            top : bottom + 1
        ][::-1]
        ...
        slice.all_column_is_numeric_train = self.all_column_is_numeric_train[
            top : bottom + 1
        ][::-1]
        slice.train_normalized_numbers = self.train_normalized_numbers[
            top : bottom + 1
        ][::-1]
        slice.bw_train_normalized_numbers = self.bw_train_normalized_numbers[
            top : bottom + 1
        ][::-1]
        slice.symbolset_normalized_numbers = self.symbolset_normalized_numbers[
            top : bottom + 1
        ][::-1]
...
def normalize_numeric(value):
    ...
    if (
        ...
        and any(char.isalpha() for char in value) == False
    ):
...
def discover_aggregation_scope(
    csv_file,
    aggregation_rows,
    cand_subheaders,
    ...
    if len(aggregation_rows) > 0 and len(certain_data_indexes) > 0:
        ...
        if len(agg_idxs) > 0 and agg_idxs[0] != certain_data_indexes[0]:
            ...
            for aggregation_idx in agg_idxs:
                ...
                aggregation_row = csv_file.loc[[aggregation_idx]].applymap(
                    normalize_numeric
                )
                candidate_scope = csv_file.loc[
                    [
                        i
                        for i in sorted(certain_data_indexes + cand_subhead_indexes)
                        if i < aggregation_idx and i not in agg_idxs
                    ]
                ].applymap(normalize_numeric)
                ...
                if aggregation_function == "sum":
                    for i in range(1, candidate_scope.shape[0] + 1):
                        ...
                        summed_rows = candidate_scope.loc[
                            candidate_scope.index[-i:]
                        ].sum(
                            axis=0, skipna=True
                        )  # .replace({ 0:np.nan})
                        ...
                        if scope_head_idx == None:
                            ...
                            if aggregation_row.iloc[0].eq(summed_rows).any():
                                ...
                                if (
                                    i < candidate_scope.shape[0]
                                    and scope_head_idx - 1 not in cand_subhead_indexes
                                ):
                                    continue
                        ...
                        if scope_head_idx != None:
                            ...
                            if cand_scope_head in candidate_scope.index:
                                summed_rows = candidate_scope.loc[cand_scope_head:].sum(
                                    axis=0, skipna=True
                                )  # .replace({ 0:np.nan})
                            # print(f'\n\tsummed_rows1=\n{pd.DataFrame([summed_rows])}')

                            while (
                                cand_scope_head >= candidate_scope.index[0]
                                and aggregation_row.iloc[0].eq(summed_rows).any()
                            ):
                                # print(f'\ncandidate_scope = \n{candidate_scope}')
                                if cand_scope_head in candidate_scope.index:
                                    summed_rows = candidate_scope.loc[
                        ...
                        if scope_head_idx != None:
                            ...
                            aggregation_rows[aggregation_idx][
                                "scope_head"
                            ] = scope_head_idx
                            ...
                            for cand_subheader_idx in cand_subheader_rev:
                                ...
                                if (
                                    value.lower()
                                    in aggregation["aggregation_label"].lower()
                                ):
                                    ...
                                    aggregation_rows[aggregation_idx]["scope_head"] = (
                                        cand_subheader_idx + 1
                                    )
                                    aggregation_rows[aggregation_idx][
                                        "scope_range"
                                    ] = list(
                            ...
                            if (
                                "context_label"
                                not in aggregation_rows[aggregation_idx].keys()
                                and last_header_value != ""
                                and last_header_value.lower()
                                in aggregation["aggregation_label"].lower()
                            ):
                            ...
                            if (
                                "subheader"
                                not in aggregation_rows[aggregation_idx].keys()
                                and cand_subhead_indexes != None
                                and aggregation_rows[aggregation_idx]["scope_head"] - 1
                                in cand_subhead_indexes
                            ):
                                # input(f'prediction is missing subheader, look for a subheader')
                                aggregation_rows[aggregation_idx]["subheader"] = (
                                    aggregation_rows[aggregation_idx]["scope_head"] - 1
                                )
                                subheader_scope[
                                    aggregation_rows[aggregation_idx]["scope_head"] - 1
                                ] = list(
                                ...
                                if (
                                    aggregation_rows[aggregation_idx]["scope_head"] - 1
                                    not in predicted_subheaders
                                ):
                                    predicted_subheaders.append(
                                        aggregation_rows[aggregation_idx]["scope_head"]
                                        - 1
                                    )
                                if (
                                ...
                                for cand_subheader_idx in cand_subheader_rev:
                                    ...
                                    if (
                                        ...
                                        aggregation_rows[aggregation_idx][
                                            ...
                                            range(
                                                cand_subheader_idx + 1, aggregation_idx
                                            )
                                        )
                                        break
                        ...
                        if (
                            cand_subhead_indexes != None
                            and candidate_scope.index[-i] - 1 in cand_subhead_indexes
                        ):
                            scope_head_idx = candidate_scope.index[-i]
                            candidate_subheader_value = str(
                                csv_file.loc[scope_head_idx - 1].tolist()[0]
                            )
                            if (
                                candidate_subheader_value.lower()
                                in aggregation["aggregation_label"].lower()
                            ):
                                ...
                                subheader_scope[scope_head_idx - 1] = list(
                                    range(scope_head_idx, aggregation_idx)
                                )
                                ...
                                if scope_head_idx - 1 in cand_subhead_indexes:
                                    ...
                                    aggregation_rows[aggregation_idx][
                                        "scope_range"
                                    ] = list(range(scope_head_idx, aggregation_idx))
                                    for di in aggregation_rows[aggregation_idx][
                                        "scope_range"
                                    ]:
                                        if di in cand_subhead_indexes:
                                            cand_subhead_indexes.remove(di)
...
def contains_number(line):
    ...
    for value in line:
        _, symbols, _, _, _ = pat.generate_pattern_symbols_and_case(
            str(value).strip(), True
        )
        if symbols != set() and symbols.issubset(
            set(["D", ".", ",", "S", "-", "+", "~"])
        ):
...
def aggregation_first_value_of_row(row_values):
    ...
    if len(row_values) > 1:
        ...
        for aggregation_keyword in [
            "total"
        ]:  # , 'average', 'agv', 'mean', 'percentage', '(%)', 'difference'
...
def assess_non_data_line(
    ...
    if (
        row_index + 1 != dataframe.shape[0]
        and len(row_values) > 0
        and row_values[0].lower() in ["", "none", "nan"]
    ):
        if len(row_values) > 1:
            DATA = False
            for value_index, value in enumerate(row_values[1:]):
                column = dataframe.iloc[
                    row_index : min(row_index + 5, dataframe.shape[0]),
    ...
    if (
        ...
        and len([i for i in row_values if i.lower() not in ["", "none", "nan"]]) <= 2
    ):
        ...
        if (
            len(row_values) > 0
            and row_values[-1].lower() == "#ref!"
            and "FOOTNOTE" not in not_data_line_events
        ):
            not_data_line_events.append("FOOTNOTE")
...
def metadata_header_keywords(row_values):
    ...
    for value in row_values:
        if value in pat.metadata_table_header_keywords:
            ...
            break
        if value in [
            "_".join((value).split(" ")) for value in pat.metadata_table_header_keywords
        ]:
            fired = True
            break
        if value in [
            "".join((value).split(" ")) for value in pat.metadata_table_header_keywords
        ]:
            fired = True
            break
    return fired
...
def predict_line_label(data_confidence, not_data_confidence):
    ...
    if data_confidence <= not_data_confidence:
        line["label"] = "NOT_DATA"
        line["value"]["confusion_index"] = confusion_index(
            not_data_confidence, data_confidence
        )
...
def predict_combined_data_confidences(
    dataframe, data_confidence, not_data_confidence, max_candidates
):
    ...
    for row_index in dataframe.index:
        ...
        if row_counter == offset + max_candidates:
            break
        if (
            row_index not in data_confidence.keys()
            or row_index not in not_data_confidence.keys()
        ):
            break
        prediction, line_is_data_conf = predict_line_label(
            data_confidence[row_index], not_data_confidence[row_index]
        )
...
def predict_line_confidences(
    ...
    (
        ...
        not_data_weights,
        max_candidates,
        summary_population_factor,
        weight_input,
        impute_nulls,
        max_summary_strength,
        max_line_depth,
        max_attributes,
        ignore_left,
        p,
    ...
    for row_index, candidate_row in dataframe.iterrows():
        ...
        candidate_row_values = [
            str(elem) if elem is not None else elem for elem in candidate_row.tolist()
        ]
        ...
        for column in dataframe:
            ...
            summary_strength = line_agreements[row_index][columnindex][
                "summary_strength"
            ]
            ...
            if (
                ...
                and (
                    ...
                    or line_agreements[row_index][columnindex]["summary_strength"] == 1
                )
                and impute_nulls == True
                and row_index - 1 in line_agreements.keys()
                and columnindex in line_agreements[row_index - 1].keys()
                and row_index - 1 in data_line_confidences.keys()
                and data_line_confidences[row_index - 1]
                > not_data_line_confidences[row_index - 1]
            ):
                value_agreements = line_agreements[row_index - 1][columnindex][
                    "agreements"
                ]
                summary_strength = line_agreements[row_index - 1][columnindex][
                    "summary_strength"
                ]
            if (
                row_counter > 0
                and line_agreements[row_index][columnindex]["summary_strength"] == 0
                and line_agreements[row_index][columnindex]["aggregate"]
                and row_index - 2 in line_agreements.keys()
                and columnindex in line_agreements[row_index - 2].keys()
                and row_index - 2 in data_line_confidences.keys()
                and data_line_confidences[row_index - 2]
                > not_data_line_confidences[row_index - 2]
            ):
                value_agreements = line_agreements[row_index - 2][columnindex][
                    "agreements"
                ]
                summary_strength = line_agreements[row_index - 2][columnindex][
                    "summary_strength"
                ]
            ...
            (
                ...
                cand_num_tokens,
                cand_num_chars,
            ) = pat.generate_pattern_symbols_and_case(
                candidate_value.strip(), outlier_sensitive
            )
            column_patterns = copy.deepcopy(
                sample_patterns[columnindex][row_index + 1 :]
            )
            ...
            column_lengths = copy.deepcopy(
                sample_chars_len[columnindex][row_index + 1 :]
            )
            ...
            if normalize_decimals == True:
                (
                    ...
                    column_patterns,
                    column_symbols,
                ) = pat.normalize_decimals_numbers_predata(
            ...
            (
                value_pattern_summary,
                value_chain_consistent,
            ) = pat.generate_pattern_summary(column_patterns)
            disagreement_summary_strength = sum(
                1 for x in column_patterns if len(x) > 0
            )
            ...
            if len(cand_pattern) > 0:
                columnvalues = dataframe.iloc[
                    row_counter:, dataframe_labels.index(column)
                ].tolist()
                if len(columnvalues) > 1:
                    neighbor = columnvalues[1]
                    repetitions_of_candidate = np.count_nonzero(
                        columnvalues[1:] == candidate_value
                    )
                    if len(columnvalues) > 2:
                        repetitions_of_neighbor = np.count_nonzero(
                            columnvalues[2:] == neighbor
                        )
                    else:
                        repetitions_of_neighbor = 0
                else:
                ...
                if disagreement_summary_strength > 0 and (
                    ...
                    or is_number(cand_symbols) == False
                ):
                    value_disagreements = find_disagreements(
                        repetitions_of_candidate,
                        repetitions_of_neighbor,
                        neighbor,
                        value_pattern_summary,
            ...
            if (
                row_index + 1 in line_agreements.keys()
                and columnindex in line_agreements[row_index + 1].keys()
            ):
                value_below_agreements = line_agreements[row_index + 1][columnindex][
                    "agreements"
                ]
                for agreement in value_below_agreements:
                    if (
                        line_agreements[row_index][columnindex]["null_equivalent"]
                        == False
                        and line_agreements[row_index][columnindex]["aggregate"]
                        == False
                        and agreement
                        not in line_agreements[row_index][columnindex]["agreements"]
                        and agreement in not_data_value_rules.keys()
                    ):
                        value_disagreements.append(agreement)

            not_data_score = max_score(
                value_disagreements, not_data_weights, not_data_weight_lower_bound
            )
        ...
        if len(candidate_row_values) > 0 and weight_input == "values_and_lines":
            ...
            arithmetic_events_fired = collect_arithmetic_events_on_row(
                candidate_row_values
            )
            ...
            for header_event in header_events_fired:
                ...
                if (
                    not_data_weights[header_event] != None
                    and not_data_weights[header_event] > not_data_weight_lower_bound
                ):
                    line_not_data_evidence.append(not_data_weights[header_event])
            if len(arithmetic_events_fired) > 0:
                arithmetic_event_score = max_score(
            ...
            if dataframe.shape[1] > 1:
                ...
                if (
                    ...
                    not in not_data_line_rules_fired
                ):
                    before_data = False
                for event in not_data_line_rules_fired:
                    line_disagreements[row_index]["line"].append(event)
                    if (
                        not_data_weights[event] != None
                        and not_data_weights[event] > not_data_weight_lower_bound
                    ):
        ...
        if len(candidate_row_values) > 0 and weight_input == "values_and_lines":
            ...
            for rule in line_is_data_events:
                ...
                if (
                    data_weights[rule] != None
                    and data_weights[rule] > weight_lower_bound
                ):
...
def find_agreements(
    ...
    if len(columnvalues) > 1:
        candidate_count = columnvalues[
            1 : min(max_values_lookahead, len(columnvalues))
        ].count(candidate)
    ...
    if len(columnvalues) > 2:
        candidate_count = columnvalues[
            2 : min(max_values_lookahead, len(columnvalues))
        ].count(candidate)
        if candidate_count == 1:
    ...
    if len(columnvalues) > 2 and partof_multiword_value_repeats_once(
        candidate_tokens,
        column_tokens[2 : min(max_values_lookahead, len(columnvalues))],
    ):
        # Only one alphabetic token from multiword value repeats below, and it repeats only once
        agreements.append("Rule_2_a")

    if len(columnvalues) > 2 and partof_multiword_value_repeats_at_least_k(
        candidate_tokens,
        column_tokens[2 : min(max_values_lookahead, len(columnvalues))],
        2,
    ):
...
def find_line_agreements(
    dataframe, dataframe_patterns, args, start_index, do_ignore_left
):
    (
        max_data,
        max_summary_strength,
    ...
    for row_index, cand_row in dataframe.iterrows():
        ...
        row_values = [
            str(elem) if elem is not None else elem for elem in cand_row.tolist()
        ]
        ...
        for column in dataframe:
            ...
            line_agreements[row_index][columnindex]["aggregate"] = len(
                value_tokens
            ) > 0 and not set(value_tokens).isdisjoint(pat.aggregation_tokens)
                ...
                for nonempty_patterns_idx in range(
                    ...
                    if len(column_patterns[nonempty_patterns_idx]) > 0:
                        ...
                        if nonempty_patterns == max_summary_strength:
                            ...
                            break
            if normalize_decimals == True:
                column_patterns, column_symbols = pat.normalize_decimals_numbers(
                    column_patterns, column_symbols
                )
            ...
            line_agreements[row_index][columnindex]["summary_strength"] = sum(
                1 for x in column_patterns if len(x) > 0
            )
...
def discover_delimiter(filepath, encoding):
    ...
    if len(delim_count.values()) > 0:
        ...
        cand_delims = [
            key for key in delim_count.keys() if delim_count[key] == maxValue
        ]
        # input(cand_delims)
        if len(cand_delims) == 0:
            ...
            return delimiter
        if (
            delimiter_idx_of_first_consistent_row[cand_delims[0]]
            < delimiter_idx_of_first_consistent_row[cand_delims[1]]
        ):
            delimiter = cand_delims[0]
            # print('Detected delimiter = "'+delimiter+'"')
            return delimiter
...
def match_sequence(
    legal_label_sequence,
    line_predictions,
    offset,
    first_double_data_index,
    data_window,
    ...
    for legal_label_idx, legal_label in enumerate(legal_label_sequence):
        ...
        if line_predictions[offset + legal_label_idx]["label"] == legal_label:
            ...
            match_weight += line_predictions[offset + legal_label_idx]["value"][
                label_weight
            ]
        else:
            # print(f"match_weight={match_weight}-{line_predictions[offset+legal_label_idx]['value'][label_weight]}")
...
def predict_last_data_line(dataframe, predicted_fdl):
            ...
            if idx[0] == dataframe.index[-1]:
                ...
                for row in nans.itertuples():
                    ...
                    if len(first_value) > 5 and (
                        (
                            (first_value[0].isdigit() or first_value[0] == "a")
    ...
    if footnote_start == None:
        for row in dataframe[::-1].itertuples():
            ...
            if len(row_values) > 0:
                ...
                for (
                    aggregation_phrase
                ) in pat.aggregation_functions:  # ['total', 'average', 'subtotal']
                ...
                if (
                    ...
                    and len(
                        [
                            ...
                            if str(i).strip().lower() not in ["", "nan", "none", "null"]
                        ]
                    )
                    == 0
                    and str(row_values[-1]).strip().lower()
                ...
                if (
                    len(first_value) > 1
                    and first_value[0].isdigit()
                    and first_value[1] not in [".", "*", ":", " ", "/", ")", ","]
                ):
                    predicted_last_data_line_found = True
                    predicted_last_data_line = row_index
                    break
    ...
    if (
        predicted_last_data_line != -1
        and predicted_last_data_line < dataframe.shape[0] - 1
    ):
        predicted_footnotes = list(
            range(predicted_last_data_line + 1, dataframe.shape[0])
        )
...
def predict_fdl(
    dataframe,
    line_predictions,
    markov_approximation_probabilities,
    markov_model,
    data_window=2,
    ...
    if len(line_predictions) > 1:
        ...
        ordered_prediction_labels = [
            line_predictions[line_index]["label"]
            for line_index in sorted(line_predictions.keys())
        ...
        if first_data_window_index >= 0:
            for sequence_id in range(0, k + 1):
                    ...
                    if markov_model != None:
                        if len(legal_sequences[sequence_id]) == 1:
                            ...
                            if markov_model == "first_order":
                                ...
                                markov_model == "second_order"
                                and len(legal_sequences[sequence_id]) == 2
                            ):
                                sequence_prior = (
                                    sequence_prior
                                    * markov_approximation_probabilities[
                ...
                while (
                    len(legal_sequences[sequence_id]) >= k - sequence_id
                    and len(legal_sequences[sequence_id]) < k
                ):
                    ...
                    if markov_model != None:
                        if len(legal_sequences[sequence_id]) == 1:
                            ...
                            if (
                                markov_model == "first_order"
                                and legal_sequences[sequence_id].count("DATA") == 1
                            ):
                                sequence_prior = (
                                    sequence_prior
                                    * markov_approximation_probabilities[
                                ...
                                markov_model == "second_order"
                                and len(legal_sequences[sequence_id]) == 2
                                and legal_sequences[sequence_id].count("DATA") == 2
                            ):
                                sequence_prior = (
                                    sequence_prior
                                    * markov_approximation_probabilities[
                                ...
                                markov_model == "second_order"
                                and len(legal_sequences[sequence_id]) > 2
                                and legal_sequences[sequence_id].count("DATA") >= 3
                            ):
                                sequence_prior = (
                                    sequence_prior
                                    * markov_approximation_probabilities[
            ...
            for sequence_id, legal_label_sequence in legal_sequences.items():
                match_weight[sequence_id] = match_sequence(
                    legal_label_sequence,
                    line_predictions,
                    offset,
            ...
            for sequence_id, sequence in legal_sequences.items():
                sequence_final_probability[sequence_id] = (
                    legal_sequence_priors[sequence_id] * match_softmax[sequence_id]
                )
            ...
            sorted_id_weight = sorted(
                sequence_final_probability.items(), key=lambda kv: (-kv[1], -kv[0])
            )
    ...
    avg_confidence, min_confidence = first_data_line_confidence(
        line_predictions, first_data_line
    )
...
def predict_fdl_old(
    ...
    for row_index, candidate_data_row in dataframe.iterrows():
        ...
        if (
            first_data_line == -1
            and row_index in not_data_confidence.keys()
            and not_data_confidence[row_index] == 0
        ):
            first_data_line = row_index
        # this line is data
        if (
            row_index in combined_data_line_confidences.keys()
            and combined_data_line_confidences[row_index] > 0
        ):
            if (
                len(candidate_data_row)
                == sum(
                    [
                        1
                        for i in candidate_data_row
                        if str(i).strip().lower() in ["", "nan"]
                    ]
                )
                == 0
            ):
            ...
            if (
                row_index + 1 in combined_data_line_confidences.keys()
                and combined_data_line_confidences[row_index + 1] == 0
            ):
                Data_diff = data_confidence[row_index] - not_data_confidence[row_index]
                NotData_diff = (
                    not_data_confidence[row_index + 1] - data_confidence[row_index + 1]
                )
...
def last_data_line_confidence(line_predictions, predicted_boundary, max_window=4):
    ...
    for method in ["confidence"]:
            ...
            if index > predicted_boundary:
                ...
                if index <= predicted_boundary - max_window:
                    ...
                    if line_predictions[index]["label"] == "NOT_DATA":
                        predicted_not_data.append(
                            line_predictions[index]["value"][method]
                        )
        ...
        if (data_window_weight + not_data_window_weight) > 0:
            avg_confidence[method] = max(
                0,
                (
                    data_window_weight * (avg_predicted_data)
                    + not_data_window_weight * (avg_predicted_not_data)
                )
                / (data_window_weight + not_data_window_weight),
            )
        else:
...
def first_data_line_confidence(line_predictions, first_data_line, max_window=4):
    ...
    for method in ["confusion_index", "confidence", "difference"]:
        ...
        if first_data_line != -1:
            ...
            if len(predicted_not_data) > 0:
                avg_predicted_not_data = sum(predicted_not_data) / len(
                    predicted_not_data
                )
            ...
            if (data_window_weight + not_data_window_weight) > 0:
                ...
                min_confidence[method] = max(
                    0,
                    (
                        data_window_weight * (min_predicted_data)
                        + not_data_window_weight * (min_predicted_not_data)
                    )
                    / (data_window_weight + not_data_window_weight),
                )
            else:
            ...
            if len(sorted_indexes) > 1:
                ...
                for index in sorted_indexes[:-1]:
                    ...
                    if line_predictions[index + 1]["label"] == "NOT_DATA":
                        predicted_not_data.append(
                            line_predictions[index + 1]["value"][method]
                        )
                    else:
                        predicted_not_data.append(
                            -line_predictions[index + 1]["value"][method]
    ...
    if len(
        ...
        if len(candidate_header) == 3:
            if len(
                [
                    i
                    for i in candidate_header[1:]
                    if str(i).strip().lower() not in ["", "nan", "none", "null"]
                ]
            ) == len(candidate_header[1:]):
                # input('>3')
                return True
            else:
                return False
        ...
        if len(candidate_header) == 4:
            if len(
                [
                    i
                    for i in candidate_header[2:]
                    if str(i).strip().lower() not in ["", "nan", "none", "null"]
                ]
            ) == len(candidate_header[2:]):
                # input('>4a')
                return True
            elif len(
                [
                ...
                len(
                    [
                        i
                        for i in candidate_header[1:-1]
                        if str(i).strip().lower() not in ["", "nan", "none", "null"]
                    ]
                )
                == len(candidate_header[1:-1])
                or len(
                    [
                        i
                        for i in candidate_header[1:]
        ...
        if len(candidate_header) > 4 and len(
            [
                i
                for i in candidate_header[2:-2]
                if str(i).strip().lower() not in ["", "nan", "none", "null"]
            ]
        ) == len([i for i in candidate_header[2:-2]]):
            # input('5')
            return True
        else:
            return False
...
def combo_row(rows):
    ...
    for csv_column, column in column_names.items():
        combo_row.append(
            (
                " ".join([cell["value"].lower() for cell in column["column_header"]])
            ).strip()
        )
    return combo_row
...
def name_table_columns(rows):
    rows = rows.applymap(
        lambda v: pat.dequote(str(v).strip())
    ...
    for rowidx, row in rows.iterrows():
        ...
        if rowidx == rows.index[0]:
            ...
            for csv_index, value in rows.loc[previous_row].items():
                ...
                if csv_index != rows.index[-1] and (
                    rows.columns.get_indexer([csv_index]) + 1
                )[0] < len(rows.columns):
                    next_csv_column = rows.columns[
                        (rows.columns.get_indexer([csv_index]) + 1)[0]
                    ]
                    while rows.loc[previous_row, next_csv_column] == "":
                        ...
                        if (rows.columns.get_indexer([next_csv_column]) + 1)[0] < len(
                            rows.columns
                        ):
                            next_csv_column = rows.columns[
                                (rows.columns.get_indexer([next_csv_column]) + 1)[0]
                            ]
                        else:
                            break
            ...
            for csv_index, value in row.items():
                if (
                    sum(
                        cell["value"] != ""
                        for cell in column_names[csv_index]["column_header"]
                    )
                    == 0
                ):
                    if csv_index in buffer_row.keys():
                        column_names[csv_index]["column_header"].append(
                            buffer_row[csv_index]
                        )
                if value != "":
                    column_names[csv_index]["column_header"].append(
                        {
...
def predict_header_indexes(file_dataframe, first_data_line_annotated, table_counter):
    ...
    data = file_dataframe.loc[
        first_data_line_annotated : min(last_row_label, first_data_line_annotated + 10)
    ]
    # print(data)
    ...
    if candidate_headers.shape[0] > 0:
        ...
        for reverse_index in range(1, candidate_headers.shape[0] + 1):
            ...
            if (
                len(candidate_headers.iloc[-reverse_index:].dropna(how="all", axis=0))
                > 6
            ):
                break
            ...
            if (
                ...
                and str(row_values[0]).strip().lower()
                not in ["", "nan", "none", "null"]
                and len(
                    [
                        i
            ...
            consider_header_dataframe = candidate_headers.iloc[-reverse_index:].drop(
                empty_lines, axis=0
            )
            ...
            if (
                ...
                for value_index, value in enumerate(row_values[2:]):
                    if str(value).strip().lower() not in [
                        ...
                        candidate_headers.iloc[-reverse_index + 1].tolist()[
                            value_index + 2
                        ]
                    ).strip().lower() in [
                        "",
                        ...
                        if len(row_values) > 4 and (
                            value_index == len(row_values[2:]) - 1
                            or value_index == len(row_values[2:]) - 2
                        ):
                            continue
                        extension = False
                        break
                if extension == True:
                    ...
                    predicted_header_indexes = [
                        x for x in list(header.index) if x not in empty_lines
                    ]
            ...
            if len(predicted_header_indexes) > 0 and (
                has_duplicates(consider_header_dataframe) == False
            ):
                NON_DUPLICATE_HEADER_ACHIEVED = True
        ...
        if len(predicted_header_indexes) > 0:
            ...
            if len(predicted_header_indexes) > 0:
                ...
                while len(predicted_header_indexes) > 0:
                    first_value = str(
                        file_dataframe.loc[last_header_index, file_dataframe.columns[0]]
                    ).strip()
                    if (
                        len(dataframe.columns) > 1
                        and file_dataframe.loc[last_header_index, 1:]
                        .isnull()
                        .values.all()
                        == True
                        and (first_value.startswith("(") and first_value.endswith(")"))
                        == False
                    ):
            ...
            if (
                len(predicted_header_indexes) == 0
                and table_counter == 1
                and first_data_line_annotated > 0
                and len(candidate_headers) > 0
            ):
                count_offset = 0
                for reverse_index in range(1, candidate_headers.shape[0] + 1):
                    count_offset += 1
                    ...
                    if (
                        len(
                            [
                                i
                                for i in row_values
                                if str(i).strip().lower()
                                not in ["", "nan", "none", "null"]
                            ]
                        )
                        > 0
                    ):
                        predicted_header_indexes.append(
                            first_data_line_annotated - count_offset
                        )
                        break
...
def discover_next_table_temp(
    ...
    for row_index in csv_file.index:
        ...
        for column_index in csv_file.columns:
            ...
            summary_strength = line_agreements[row_index][column_index][
                "summary_strength"
            ]
            ...
            # see impute agreements
            if (
                row_index in line_agreements.keys()
                and (
                    line_agreements[row_index][column_index]["null_equivalent"] == True
                    or line_agreements[row_index][column_index]["summary_strength"] == 1
                )
                and args.impute_nulls == True
                and row_index - 1 in line_agreements.keys()
                and column_index in line_agreements[row_index - 1].keys()
                and row_index - 1 in data_line_confidences.keys()
                and data_line_confidences[row_index - 1]
                > not_data_line_confidences[row_index - 1]
            ):
                value_agreements = line_agreements[row_index - 1][column_index][
                    "agreements"
                ]
                summary_strength = line_agreements[row_index - 1][column_index][
                    "summary_strength"
                ]
            if (
                row_index in line_agreements.keys()
                and line_agreements[row_index][column_index]["summary_strength"] == 0
                and line_agreements[row_index][column_index]["aggregate"]
                and row_index - 2 in line_agreements.keys()
                and column_index in line_agreements[row_index - 2].keys()
                and row_index - 2 in data_line_confidences.keys()
                and data_line_confidences[row_index - 2]
                > not_data_line_confidences[row_index - 2]
            ):
                value_agreements = line_agreements[row_index - 2][column_index][
                    "agreements"
                ]
                summary_strength = line_agreements[row_index - 2][column_index][
                    "summary_strength"
                ]
            ...
            data_score = max_score(
                value_agreements, rule_weights["data_cell"], args.weight_lower_bound
            )
            POPULATION_WEIGHT = 1 - (1 - args.p) ** (2 * summary_strength)
            if data_score != None:
                if args.summary_population_factor:
            ...
            #######################################################################v######
            #  NOT DATA value classification
            value_disagreements = line_disagreements[row_index][column_index][
                "disagreements"
            ]
            disagreement_summary_strength = line_disagreements[row_index][column_index][
                "disagreement_summary_strength"
            ]
            not_data_score = max_score(
                value_disagreements,
                rule_weights["not_data_cell"],
            ...
            if csv_file.shape[1] > 1:
                ...
                for event in not_data_line_rules_fired:
                    ...
                    if (
                        ...
                        and rule_weights["not_data_line"][event] == None
                    ):
                        steps = event[-1]
                        if int(steps) in range(2, 6):
                            event = event[:-1] + str(int(steps) + 1)
                    if (
                        rule_weights["not_data_line"][event] != None
                        and rule_weights["not_data_line"][event]
                        > args.not_data_weight_lower_bound
                    ):
                        line_not_data_evidence.append(
                            rule_weights["not_data_line"][event]
                        )
        ...
        if args.weight_input == "values_and_lines":
            ...
            for rule in line_is_data_events:
                if (
                    rule_weights["data_line"][rule] != None
                    and rule_weights["data_line"][rule] > args.weight_lower_bound
                ):
    ...
    (
        ...
        csv_file, data_line_confidences, not_data_line_confidences, args.max_candidates
    )
    # if table_counter>1:
...
def process_dataframe(table_counter, input_dataframe, args, delimiter):
    ...
    if pat_first_data_line != -1:
        ...
        predicted_table["fdl_confidence"]["softmax"] = float(
            first_data_line_combined_data_predictions["softmax"]
        )
        ...
        while (
            last_data_line_index in combined_data_line_confidences.keys()
            and line_predictions[last_data_line_index]["label"] == "DATA"
        ):
        ...
        predicted_header_indexes, predicted_pat_subheaders = predict_header_indexes(
            file_dataframe.loc[:last_data_line_index],
            pat_first_data_line,
            table_counter,
        )
        ...
        cleaned_candidate_data = input_dataframe.loc[
            pat_first_data_line:last_data_line_index, :
        ...
        if len(predicted_header_indexes) > 0:
            ...
            cleaned_candidate_section = input_dataframe.loc[
                predicted_header_indexes[0] : last_data_line_index, :
            ].dropna(axis=1, how="all", inplace=False)
            predicted_table["header"]["combo"] = combo_row(
                file_dataframe.loc[predicted_header_indexes, :]
            )
            predicted_table["five_lines"] = cleaned_candidate_section.loc[
                pat_first_data_line : min(
                    cleaned_candidate_section.index[-1], pat_first_data_line + 5
                ),
                :,
            ].to_csv(sep=",", index=False, header=False)
        else:
            cleaned_candidate_section = cleaned_candidate_data.dropna(
                axis=1, how="all", inplace=False
            )
            predicted_table["five_lines"] = cleaned_candidate_section.iloc[
                : min(cleaned_candidate_section.shape[0], 5), :
            ].to_csv(sep=",", index=False, header=False)
        ...
        if (
            cleaned_candidate_data.shape[0] == 1
            and len(predicted_header_indexes) == 0
            or cleaned_candidate_section.shape[1] == 1
        ):
            # print('Invalid Data')
            table_counter -= 1
        else:
            ...
            if (len(predicted_header_indexes) == 0 and pat_first_data_line != 0) or (
                len(predicted_header_indexes) > 0
                and top_context_boundary != predicted_header_indexes[0]
            ):
                ...
                premeta_text = file_dataframe.iloc[
                    top_context_boundary:meta_end, :
                ].fillna("")
                predicted_table["premeta"][
                    "premeta_text"
                ] = premeta_text.values.tolist()

            # print('\n---------------\n-- DATA\n---------------\n')
            # print(cleaned_candidate_data.dropna(axis=1, how='all',inplace=False))
            predicted_table["dataframe"] = cleaned_candidate_data.dropna(
                axis=1, how="all", inplace=False
            )
...
def view_dataframe(filepath):
    ...
    if failure == None and all_csv_tuples != None and len(all_csv_tuples) > 0:
        ...
        while line_index < len(all_csv_tuples):
            ...
            if len(csv_tuple) != num_fields or (
                len(csv_tuple) == num_fields and line_index + 1 == len(all_csv_tuples)
            ):
    ...
    if failure == None and all_csv_tuples != None and len(all_csv_tuples) > 0:
        ...
        for dataframe in dataframes:
            ...
            try:
                ...
                table_counter, tables, predicted_combined_data = process_dataframe(
                    table_counter, dataframe, args, discovered_delimiter
                )
                # pp.pprint(tables)
