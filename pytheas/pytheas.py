        ...
        if failure == None:
            try:
                with codecs.open(filepath, "rU", encoding=discovered_encoding) as f:
                    ...
                    predictions = pytheas_model.extract_tables(
        ...
        with conn.begin():
            ...
            if predictions != None:
                ...
                for table_id in predictions:
                    ...
                    table_dataframe = table_dataframe.dropna(
                    ...
                    first_5_rows = str(
                        [tuple(x) for x in table_dataframe.head().values]
                    )

                    first_data_line_confidence = float(
                    ...
                    null_columns = table_dataframe.columns[
                        table_dataframe.isna().all()
                    ].tolist()
                    table_dataframe = table_dataframe.drop(null_columns, axis=1)
                    column_names = name_table_columns(
                        table_dataframe.loc[header_lines]
                    )  # apply combination algorithm to values in discovered header section
                    ...
                    predictions[table_id]["header_lines"] = header_dataframe.to_json(
                        orient="split"
                    )
                    ...
                    first_data_line_avg_difference = float(
                        round(table["fdl_confidence"]["avg_difference"], 4)
                    )
                    ...
                    if len(table["footnotes"]) > 0:
                        footnotes_idx = list(
                            file_dataframe.loc[table["footnotes"]]
                    ...
                    context = list(
                        set(range(top_boundary, from_index)) - set(header_lines)
                    )
                    if len(context) > 0:
                        context_dataframe = file_dataframe.loc[context]
                        context_idx = list(
                            context_dataframe.dropna(how="all", axis=0).index
                        )
...
class PYTHEAS:
    ...
    def process_endpoint(
        ...
        with opendata_engine.connect() as conn:
            endpoint_data = pd.read_sql_query(
                sql=f"""
            ...
            """,
                con=conn,
                params=(", ".join(endpoint), max_lines, max_lines),
            )
            # AND is_processed = False
            # ORDER BY size_bytes

            files = [
                [x.crawl_datafile_key, x.size_in_bytes, x.path, x.endpoint_dbname]
                for x in endpoint_data.itertuples()
            ]
            ...
            print(
                f"Completed {processed} of {NINPUTS} files ({processed/NINPUTS}%) :deal-with-it-parrot:."
            )
    ...
    def collect_rule_activation(self, db_cred, num_processors, top_level_dir):
        ...
        with Pool(processes=NPROC) as pool:
            with tqdm(total=NINPUTS) as pbar:
                for r in pool.imap_unordered(
                    pat_rule_worker,
                    generate_rule_annotation_tasks(self, top_level_dir, db_cred),
                ):
                    combined_data.append(r)
                    pbar.update(1)
        ...
        execute_values(
            cur,
            """INSERT INTO pat_line_and_cell_rules (
                        crawl_datafile_key, data_rules_fired, not_data_rules_fired) 
                        VALUES %s""",
            pat_line_and_cell_rules_DATA,
        )
        ...
        pat_line_datapoints.head(0).to_sql(
            "pat_line_datapoints", engine, if_exists="replace", index=False
        )  # truncates the table
        ...
        pat_cell_datapoints.head(0).to_sql(
            "pat_cell_datapoints", engine, if_exists="replace", index=False
        )  # truncates the table
        conn = engine.raw_connection()
        cur = conn.cursor()
        output = io.StringIO()
        ...
        pat_data_line_rules.head(0).to_sql(
            "pat_data_line_rules", engine, if_exists="replace", index=False
        )  # truncates the table
        conn = engine.raw_connection()
        cur = conn.cursor()
        output = io.StringIO()
        ...
        pat_not_data_line_rules.head(0).to_sql(
            "pat_not_data_line_rules", engine, if_exists="replace", index=False
        )  # truncates the table
        conn = engine.raw_connection()
        cur = conn.cursor()
        output = io.StringIO()
        ...
        contents = output.getvalue()
        cur.copy_from(
            output, "pat_not_data_line_rules"
        ...
        pat_data_cell_rules.head(0).to_sql(
            "pat_data_cell_rules", engine, if_exists="replace", index=False
        )  # truncates the table
        conn = engine.raw_connection()
        cur = conn.cursor()
        output = io.StringIO()
        ...
        pat_not_data_cell_rules.head(0).to_sql(
            "pat_not_data_cell_rules", engine, if_exists="replace", index=False
        )  # truncates the table
        conn = engine.raw_connection()
        cur = conn.cursor()
        output = io.StringIO()
        ...
        contents = output.getvalue()
        cur.copy_from(
            output, "pat_not_data_cell_rules"
        )  # , null="" null values become ''
        conn.commit()
        end = timer()
        ...
        cur = con.cursor()
        cur.execute(
            """CREATE INDEX IF NOT EXISTS pat_cell_datapoints_idx on pat_cell_datapoints(crawl_datafile_key)"""
        )
        cur.execute(
            """CREATE INDEX IF NOT EXISTS pat_line_datapoints_idx on pat_line_datapoints(crawl_datafile_key)"""
        )

        cur.execute(
            """CREATE INDEX IF NOT EXISTS pat_data_cell_rules_idx on pat_data_cell_rules(crawl_datafile_key)"""
        )
        cur.execute(
        ...
        cur.execute(
            """CREATE INDEX IF NOT EXISTS pat_data_line_rules_idx on pat_data_line_rules(crawl_datafile_key)"""
        )
        cur.execute(
...
def koci_to_pat_input(crawl_datafile_key, db_cred):
    ...
    line_weights = pd.DataFrame(
        index=file_lines, columns=["DATA", "NOT_DATA", "majority_label"]
    )
    for line_index in file_lines:
        ...
        if blank_flag:
            ...
            not_data_cell_weights = not_data[
                ["CONTEXT", "FOOTNOTE", "HEADER", "OTHER", "SUBHEADER"]
            ].apply(sum, axis=1)
        ...
        if data_line_weight >= not_data_line_weight:
            ...
            line_predictions[line_index]["value"][
                "confusion_index"
            ] = table_classifier_utilities.confusion_index(
                data_line_weight, not_data_line_weight
            )
            line_predictions[line_index]["value"]["difference"] = (
                data_line_weight - not_data_line_weight
            )
        else:
