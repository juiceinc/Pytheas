...
cell_rules = {
    ...
    "data": {
        ...
        "CONSISTENT_NUMERIC": {"name": "Rule_4_a consistently ONE symbol = D"},
        "CONSISTENT_D_STAR": {
        ...
        "FW_D5PLUS": {"name": "Rule_13_fw FW summary is [['D',count]], where count>=5"},
        "BW_D5PLUS": {"name": "Rule_13_bw BW summary is [['D',count]], where count>=5"},
        "FW_D1": {"name": "Rule_14_fw FW summary is [['D',1]]"},
        "BW_D1": {"name": "Rule_14_bw BW summary is [['D',1]]"},
        "FW_D4": {"name": "Rule_15_fw FW summary is [['D',4]]"},
        "BW_D4": {"name": "Rule_15_bw BW summary is [['D',4]]"},
        "FW_LENGTH_4PLUS": {
        ...
        "CASE_SUMMARY_CAPS": {"name": "Rule_18 case summary is ALL_CAPS"},
...
line_rules = {
    "not_data": {
        "header": {
            ...
            #      "name":""},
            "ADJACENT_ARITHMETIC_SEQUENCE_2": {"name": ""},
            "ADJACENT_ARITHMETIC_SEQUENCE_3": {"name": ""},
            "ADJACENT_ARITHMETIC_SEQUENCE_4": {"name": ""},
            "ADJACENT_ARITHMETIC_SEQUENCE_5": {"name": ""},
            "ADJACENT_ARITHMETIC_SEQUENCE_6_plus": {"name": ""},
            "RANGE_PAIRS_1": {"name": ""},
            "RANGE_PAIRS_2_plus": {"name": ""},
            "PARTIALLY_REPEATING_VALUES_length_2_plus": {"name": ""},
            "METADATA_LIKE_ROW": {"name": ""},
            "CONSISTENTLY_SLUG_OR_SNAKE": {"name": ""},
            "CONSISTENTLY_UPPER_CASE": {},
        },
        "aggregation": {
            "AGGREGATION_ON_ROW_WO_NUMERIC": {"name": ""},
            "AGGREGATION_ON_ROW_W_ARITH_SEQUENCE": {"name": ""}
        ...
        "other": {
            ...
            "STARTS_WITH_NULL": {"name": "line starts with null"},
            "NO_SUMMARY_BELOW": {"name": "no summary achieved below in any column"},
            "FOOTNOTE": {"name": "line resembles footnote"},
        },
    },
    "data": {
        "aggregation": {
...
def normalize_decimals_numbers_predata(
    value,
    cand_pattern,
    cand_symbols,
    cand_case,
    ...
    new_summary_symbols = []
    new_value = value
    if "D" in cand_symbols and cand_symbols.issubset(
        set(["D", ".", ",", "S", "-", "+", "~", "(", ")"])
    ):
        ...
        (
            ...
            cand_num_tokens,
            cand_num_chars,
        ) = generate_pattern_symbols_and_case(new_value, outlier_sensitive)
    ...
    for pattern in summary_patterns:
        ...
        indices = [
            i for i, x in enumerate(symbolchain) if x in ["-", "+", "~", ">", "<"]
        ]
        ...
        if len(pattern) > 0:
            digits = [
                symbol_count[1] for symbol_count in pattern if symbol_count[0] == "D"
            ]
...
def split_metadata_data(csv_tuples, offset, sample_lines_limit, delimiter):
    ...
    while offset < limit:
        ...
        if (
            line in ["\n", "\r\n"]
            or len(line) == 0
            or sum(len(s.strip()) for s in line) == 0
        ):
            while (
                line in ["\n", "\r\n"]
...
def assess_candidate_header(datarow):
    ...
    if (
        len(datarow) > 2
        and str(datarow[0]).strip().lower() in ["", " ", "none", "nan"]
        and str(datarow[1]).strip().lower() in ["", " ", "none", "nan"]
    ):
        candidate = datarow[2::]
    elif len(datarow) > 1 and str(datarow[0]).strip().lower() in [
        "",
        " ",
        "none",
        "nan",
                ...
                if len(summary_pattern) > symbol_idx:  # make sure index in bounds
                    ...
                    if symbol[0] == summary_pattern[symbol_idx][0]:
                        ...
                        if (
                            ...
                            and symbol[1] != summary_pattern[symbol_idx][1]
                        ):
                            summary_pattern[symbol_idx][1] = 0
                        ...
                        if symbol_idx == len(pattern) - 1 and len(pattern) < len(
                            summary_pattern
                        ):
...
def consistent_symbol_sets_increment(consistent_ss, symbol_sig):
    ...
    if (
        len(symbol_sig) > 0
        and consistent_symbol_sets == True
        and symbol_sig != consistent_symbols
    ):
...
def discover_incremental_values(row_values, sample_symbols):
    ...
    for idx, value in enumerate(row_values):
        if value != None:
            ...
            if (value.count("-") == 0 and isfloat(value)) or (
                value.count("-") == 1
                and len(sample_symbols[idx]) > 0
                and (sample_symbols[idx][0]).issubset(set(["D", ",", ".", "-", "S"]))
                and isfloat(evaluate(value))
            ):
                try:
                    ev = evaluate(value)
                except:
    ...
    if len(numeric_value_list) > 1:
        for i in range(len(numeric_value_list)):
            ...
            for k in k_indexes:
                ...
                if value - numeric_value_list[k] > 0:
                    ...
                    if (
                        step in step_pairs.keys()
                        and numeric_value_list[len(numeric_value_list) - (i + 1)]
                        <= numeric_value_list[step_pairs[step][-1]]
                        and k < step_pairs[step][-1]
                    ):
        ...
        # print(step_counter)
        sorted_increment_counter_desc = sorted(
            step_counter.items(), key=lambda x: (-x[1], x[0])
        )
...
def identify_postmeta_candidates(dataframe):
    ...
    for index, row in dataframe.iloc[::-1].iterrows():
        ...
        if (
            len(datatuple) > 1
            and len(datatuple[0]) > 0
            and (
                "source" in (str(datatuple[0]).strip()).lower()
                or len(
                    [x for x in datatuple[1:] if x != None and len(str(x).strip()) > 0]
                )
                == 0
            )
        ):
...
def discover_header_and_datastart(
    org_dataframe, candidate_size=20, trust_threshold=5, max_sample_size=100
):
    ...
    if postmeta_cand_startindex < org_dataframe.shape[0]:
        ...
        dataframe = org_dataframe.drop(
            org_dataframe.index[
                list(range(postmeta_cand_startindex, org_dataframe.shape[0]))
            ]
        )
    ...
    for column in dataframe:
        ...
        for value in columnvalues:
            ...
            (
                ...
                value_num_tokens,
                value_num_chars,
            ) = generate_pattern_symbols_and_case(str(value).strip(), False)
    ...
    for candidate_index, cand_predata_row in candidate_pre_data_rows.iterrows():
        ...
        if candidate_index == 0:
            for column in dataframe:
                ...
                columnsummary = ColumnSampleSummary(
                    all_values_summary,
                    pattern_BW_summary,
                    ...
                    if strong_agreement:
                        ...
                        if first_data_row_confidence[candidate_index] >= 0.9:
                            ...
                            if postmeta_cand_startindex != dataframe.shape[0]:
                                dataend_index = eval_candidate_postmeta(
                                    cand_postmeta_rows, outlier_aware_summaries[0]
                                )  # pre_data_row_col_summaries[0]
                            ...
                            return (
                                ...
                                dataend_index,
                                header_index,
                                first_data_row_confidence[candidate_index],
                                first_row_data_row_column_summaries,
                                {},
                                agreeinging_columns,
                                {},
                                "data starts from first row in block",
                                null_equi_spotted,
                                sorted_rows,
                                outlier_aware_summaries,
                                outliers,
        ...
        if (
            candidate_index + trust_threshold >= row_size
        ):  ## if there are not enough rows to trust general summaries
        ...
        if last_sample_index + 1 < row_size:
            ...
            for column in dataframe:
                ...
                (
                    ...
                    value_num_tokens,
                    value_num_chars,
                ) = generate_pattern_symbols_and_case(value.strip(), False)
                ...
                sample_token_len[dataframe_labels.index(column)].append(
                    value_num_tokens
                )
        ...
        summary_strength = (
            {}
        )  # number of nonnull values that produced the summary of the column
        ...
        for column in dataframe:
            ...
            column_sample_patterns = copy.deepcopy(
                sample_patterns[dataframe_labels.index(column)]
            )
            # input('column_sample_patterns='+str(column_sample_patterns))
            pattern_FW_summary, consistent_symbol_chain = generate_pattern_summary(
                column_sample_patterns
            )
            # input('pattern_FW_summary= '+str(pattern_FW_summary))
            bw_patterns = [
                list(reversed(pattern)) for pattern in column_sample_patterns
            ]
            pattern_BW_summary, _ = generate_pattern_summary(bw_patterns)

            columns_pattern_FW_summary[
                dataframe_labels.index(column)
            ] = pattern_FW_summary
            columns_pattern_BW_summary[
                dataframe_labels.index(column)
            ] = pattern_BW_summary
            ...
            columns_symbols_summary[
                dataframe_labels.index(column)
            ] = generate_symbol_summary(sample_symbols[dataframe_labels.index(column)])
            columns_cases_summary[
                dataframe_labels.index(column)
            ] = generate_case_summary(sample_cases[dataframe_labels.index(column)])
            ...
            columnsummary = ColumnSampleSummary(
                columns_pattern_FW_summary[dataframe_labels.index(column)],
        ...
        for column in dataframe:
            ...
            if dataframe_labels.index(column) == 0 and (
                candidate_value == "" or candidate_value == None
            ):
                first_value_on_row_isempty = True

            value_pattern_summary = columns_pattern_FW_summary[
                dataframe_labels.index(column)
            ]

            value_pattern_BW_summary = columns_pattern_BW_summary[
                dataframe_labels.index(column)
            ]
            value_chain_consistency = columns_chain_consistency[
                dataframe_labels.index(column)
            ]
            value_symbol_summary = columns_symbols_summary[
                dataframe_labels.index(column)
            ]
            ...
            if (
                candidate_value == ""
                or candidate_value == None
                or str(candidate_value).strip() == ""
            ):  # in ['','data not available', 'nan','not available', 'no data', 'no answer', 'nd', 'na', 'n/d', 'n/a','not applicable', '-','--','.', '..','...', 'null', 'none']:

                if candidate_index > 0 and first_value_on_row_isempty == True:
                    prev_row = candidate_pre_data_rows[
                    ...
                    if (
                        candidate_value == ""
                        or candidate_value == None
                        or str(candidate_value).strip().lower()
                        in null_equivalent_values
                    ):
                        continue
                else:
                    continue
            ...
            (
                ...
                cand_num_tokens,
                cand_num_chars,
            ) = generate_pattern_symbols_and_case(candidate_value.strip(), False)

            if (
                len(sample_patterns[dataframe_labels.index(column)]) > 1
                and summary_strength[dataframe_labels.index(column)] >= 2
            ) or (
                len(sample_patterns[dataframe_labels.index(column)]) == 1
                and summary_strength[dataframe_labels.index(column)] == 1
            ):
                disagreements = find_disagreement(
                    value_pattern_summary,
                    value_pattern_BW_summary,
                    value_chain_consistency,
                    cand_pattern,
                ...
                if len(disagreements) > 0:
                    ...
                    confidence = disagreement_confidence(
                        disagreements, summary_strength[dataframe_labels.index(column)]
                    )
                    disagreeing_columns[dataframe_labels.index(column)][
                        "confidence"
                    ] = float(confidence)
        ...
        predata_row_confidence = calculate_row_summary_disagreement(
            disagreeing_columns, candidate_index, row_values, sample_symbols
        )
        row_agreements[candidate_index + 1] = agreeing_columns
        row_disagreements[candidate_index] = disagreeing_columns

        first_data_row_confidence[candidate_index + 1] = combined_row_confidence(
            float(predata_row_confidence), float(data_confidence), candidate_index
        )
        # LOG
        print(
            "confidence row_"
        ...
        input(
            "\n---->Row_"
            + str(candidate_index + 1)
            + " is the first data row with combined confidence: "
            + str(first_data_row_confidence[candidate_index + 1])
        )
        ...
        if first_disagreement_found == True and len(first_disagreeing_row.keys()) == 0:
            ...
            first_disagreeing_row["column_summaries"] = [
                json.dumps(x.__dict__) for x in row_column_summaries[candidate_index]
            ]  # row_column_summaries[candidate_index]

        if (
            first_data_row_confidence[candidate_index + 1] == 1
        ):  # and assess_candidate_header(candidate_pdr):
            header_index, header_detection_rule = asses_candidate_predatarow(
                candidate_pre_data_rows, candidate_index
            )
        ...
        if (
            data_confidence >= 0.9
            or (data_confidence + predata_row_confidence) / 2 >= 0.9
        ):  # and assess_candidate_header(candidate_pdr):
            # input('\ndata_confidence>=0.9\n')
    ...
    if datastart_found == False:
        ...
        sorted_rows = sorted(
            first_data_row_confidence.items(), key=lambda x: (-x[1], x[0])
        )
        if candidate_pre_data_rows.shape[0] == 1:
            ...
            for row_index, row_conf in sorted_rows:
                try:
                    nextrow = candidate_pre_data_rows[
                ...
                if row_index - 1 >= 0:
                    thisrow = candidate_pre_data_rows[
                        row_index - 1 : row_index
                    ].values.tolist()[0]
                    this_row_num_nulls = thisrow.count("")
                    header_index, header_detection_rule = asses_candidate_predatarow(
                        candidate_pre_data_rows, row_index - 1
                    )
    ...
    outlier_aware_summaries, outliers = generate_outlier_sensitive_column_summaries(
        values_dataframe[datastart_index:]
    )
    ...
    for ind, column in enumerate(dataframe.columns):
        ...
        for idx, value in values_sans_outliers.items():
            (
                ...
                value_num_tokens,
                value_num_chars,
            ) = generate_pattern_symbols_and_case(str(value).strip(), True)
...
def eval_candidate_postmeta(cand_postmeta_rows, pre_data_row_firstcol_summary):
    ...
    for rowindex, row in cand_postmeta_rows.iterrows():
        ...
        (
            ...
            num_tokens,
            num_chars,
        ) = generate_pattern_symbols_and_case(value, True)

        disagreements = find_disagreement(
            pre_data_row_firstcol_summary.fw_pattern_summary,
            pre_data_row_firstcol_summary.bw_pattern_summary,
            pre_data_row_firstcol_summary.strict_chain_summary,
            pattern,
            pre_data_row_firstcol_summary.symbol_set_summary,
            symbols,
            pre_data_row_firstcol_summary.consistent_case_summary,
            case,
        )
        ...
        if (
            len(disagreements) > 0
            or num_chars > 1.2 * pre_data_row_firstcol_summary.maxchar
            or "note" in value.lower()
            or "nota" in value.lower()
...
def asses_candidate_predatarow(candidate_pre_data_rows, candidate_index):
    ...
    if candidate_index - 1 >= 0:
        candidate_pdr = candidate_pre_data_rows[
            candidate_index - 1 : candidate_index
        ].values[0]
        # print(candidate_pdr)
        if assess_candidate_header(candidate_pdr):
            ...
            header_detection_rule = (
                "second row above the first data row qualifies as a header (-2)"
            )
    ...
    if assess_candidate_header(candidate_pdr):
        ...
        header_detection_rule = (
            "first row above the first data row qualifies as a header (-1)"
        )
    ...
    #    break
        ...
        str(candidate_pdr[0]).strip() == ""
        or str(candidate_pdr[0]).strip() == ""
        and str(candidate_pdr[1]).strip() == ""
    ):
        # print('\ntry two row combo')
        if candidate_index - 1 >= 0:
            tworow_combo = combo_row(
                candidate_pre_data_rows[candidate_index - 1 : candidate_index + 1]
            )
            if assess_candidate_header(tworow_combo):
                ...
                len(set(tworow_combo[2:])) == len(tworow_combo[2:])
                and "" not in tworow_combo[2:]
            ):
                header_index = [candidate_index - 1, candidate_index + 1]
                header_detection_rule = (
                    "two row combo with up to two spaces in the beginning"
                )
                # print(header_detection_rule)
                return header_index, header_detection_rule
        ...
        if candidate_index - 2 >= 0:
            if assess_candidate_header(
                combo_row(
                    candidate_pre_data_rows[candidate_index - 2 : candidate_index + 1]
                )
            ):
                ...
                header_index = [candidate_index + 1, candidate_index + 1]
                header_detection_rule = (
                    "no header detected in the three rows above the data"
                )
                # print(header_detection_rule)
                return header_index, header_detection_rule
        else:
    ...
    for rowidx, row in enumerate(rows):
        ...
        for idx, value in enumerate(row):
            ...
            if combo_row[idx].strip() != "":
                combo_row[idx] = (
                    dequote(combo_row[idx].strip())
                    + " "
                    + dequote(value.strip()).strip()
                )
            else:
                combo_row[idx] = (
                    buffer_row[idx].strip() + " " + dequote(value.strip()).strip()
                )
    return combo_row
...
def find_disagreement(
    ...
    if len(value_pattern_summary) != 0 and pattern_disagrees(
        value_pattern_summary, cand_pattern
    ):
    ...
    if len(value_pattern_BW_summary) != 0 and pattern_disagrees(
        value_pattern_BW_summary, list(reversed(cand_pattern))
    ):
    ...
    if value_chain_consistent and symbol_chain_disagrees(
        value_symbol_chain, cand_symbol_chain
    ):
    ...
    if len(value_symbol_summary) != 0 and symbol_summary_disagrees(
        value_symbol_summary, cand_symbols
    ):
...
def decimal_pattern(column_symbols, column_patterns):
    ...
    for value_index, value_symbols in enumerate(column_symbols):
        ...
        count_decimal = count_decimal + [
            x[0] for x in column_patterns[value_index]
        ].count(".")
        count_decimal = count_decimal + [
            x[0] for x in column_patterns[value_index]
        ].count(",")
        ...
        if (
            "D" in value_symbols
            and count_decimal == 1
            and value_symbols.issubset(set(["D", ",", ".", "-", "S"]))
            and (
                (
                    [x[0] for x in column_patterns[value_index]].count("-") == 1
                    and len(column_patterns[value_index]) > 0
                    and column_patterns[value_index][0][0] == "-"
                )
                or ([x[0] for x in column_patterns[value_index]].count("-") == 0)
            )
        ):
...
def numeric_patterns(column_symbols, column_patterns):
    ...
    for value_index, value_symbols in enumerate(column_symbols):
        ...
        if (
            ...
            numeric_spotted = (
                True  # confirm i found a number, keep checking next values
            )
...
def generate_all_numeric_sig_pattern(patterns_numeric, train_lengths):
    ...
    if len(valid_indexes) > 0:
        return (
            np.all(np.array([patterns_numeric[i] for i in valid_indexes])),
            len(valid_indexes),
        )
    return False, len(valid_indexes)


def numeric_train_incremental_pattern(
    numeric_train_sig, len_train_sig, column_is_numeric_pattern
):
...
def strong_pattern_agreement(
    ...
    for column in values_dataframe:
        ...
        if len(column_patterns) > 0 and column_patterns[0] != []:
            ...
            if (len(column_patterns) > 2 and non_empty_patterns >= 3) or (
                ...
                columnvalues = values_dataframe.iloc[
                    :, dataframe_labels.index(column)
                ].tolist()
                ...
                if (
                    consistent_symbol_chain == True
                    and len(all_values_summary) >= 3
                    and (
                        len(
                            list(
                                set([i[0] for i in all_values_summary]).difference(
                                    ["A", "D", "S", "_"]
                                )
                            )
                        )
                    ...
                    agreeing_columns[dataframe_labels.index(column)][
                        "rule"
                    ] = "strong FW pattern agreement"
                    continue

                if (
                    consistent_symbol_chain == True
                    and len(all_values_summary) >= 2
                    and len([i[1] for i in all_values_summary if i[1] >= 2]) > 0
                ):
                    strong_aggreement = True
                    agreeing_columns[dataframe_labels.index(column)] = {}
                    agreeing_columns[dataframe_labels.index(column)][
                ...
                if (
                    ...
                    agreeing_columns[dataframe_labels.index(column)][
                        "rule"
                    ] = "consistent number of digits > 3"
                    continue

                if (
                    consistent_symbol_chain == True
                    and len(all_values_summary) in [1, 2]
                    and all_values_summary[0][0] == "D"
                ):
                    agreeing_columns[dataframe_labels.index(column)] = {}
                    agreeing_columns[dataframe_labels.index(column)][
                        "confidence"
                    ] = float(0.7 * POPULATION_WEIGHT)
                    agreeing_columns[dataframe_labels.index(column)][
                        "rule"
                    ] = "up to two symbols, the first is a digit"
                    continue
                ...
                if numeric_patterns(column_symbols, column_patterns):
                    ...
                    agreeing_columns[dataframe_labels.index(column)][
                        "rule"
                    ] = "all values digits, optionally have . or , "
                    continue
                if ratio_patterns(column_symbols):
                    ...
                    agreeing_columns[dataframe_labels.index(column)][
                        "rule"
                    ] = "all values digits with \%,  optionally have . or , "
                    continue
                ...
                if len(pattern_BW_summary) >= 3 and "S" not in [
                    ...
                    continue
                if (
                    len(sample_symbols) >= 2
                    and {"S", "_"}.intersection(sample_symbols) == 0
                ):
                    agreeing_columns[dataframe_labels.index(column)] = {}
                    agreeing_columns[dataframe_labels.index(column)][
                        "confidence"
                    ] = float(0.4 * POPULATION_WEIGHT)
                    agreeing_columns[dataframe_labels.index(column)][
                        "rule"
                    ] = "at least two symbols in the symbol summary, none of which are S or _"
                    continue
                if consistent_symbol_chain == True and len(all_values_summary) >= 3:
                    agreeing_columns[dataframe_labels.index(column)] = {}
                    agreeing_columns[dataframe_labels.index(column)][
                        "confidence"
                    ] = float(0.3 * POPULATION_WEIGHT)
                    agreeing_columns[dataframe_labels.index(column)][
                        "rule"
                    ] = "three or more symbols consistent chain"
                    continue

                if consistent_symbol_chain == True and len(all_values_summary) in [
                    1,
                    2,
                ]:
                    agreeing_columns[dataframe_labels.index(column)] = {}
                    agreeing_columns[dataframe_labels.index(column)][
                        "confidence"
                    ] = float(0.2 * POPULATION_WEIGHT)
                    agreeing_columns[dataframe_labels.index(column)][
                        "rule"
                    ] = "one or two symbols consistent chain"  # eg, words
                    continue
    ...
    column_agreement_confidence = [
        d["confidence"] for d in list(agreeing_columns.values()) if "confidence" in d
    ]
    score_counts = {
        x: column_agreement_confidence.count(x) for x in column_agreement_confidence
    }
    ...
    if len(score_counts) > 0:
        ...
        if len(row_values) > 0:
            step_increment, step_count = discover_incremental_values(
                row_values[0], sample_symbols
            )
            if step_increment != 0 and step_count >= 3:
                INCREMENTAL_ATTRIBUTES_WEIGHT = a ** (step_count - 1)
                data_confidence = (
                    w * (1 - np.prod(prod_list))
                    + (1 - w) * INCREMENTAL_ATTRIBUTES_WEIGHT
                )
            else:
...
def combined_row_confidence(predata_row_confidence, data_confidence, candidate_index):
    ...
    combined_row_confidence = (
        DEPTH_WEIGHT * (predata_row_confidence + data_confidence) / 2
    )
...
def calculate_row_summary_disagreement(
    disagreeing_columns, lineindex, row_values, sample_symbols, a=0.5, method="precise"
):  # a= 0.2
    ...
    column_confidence_values = [
        d["confidence"] for d in list(disagreeing_columns.values()) if "confidence" in d
    ]
    number_of_disagreeing_attributes = len(disagreeing_columns)
    # approximation
    if len(column_confidence_values) != 0 and method == "avg":
        # using average
        predata_row_confidence = (
            1
            - (1 - (sum(column_confidence_values) / len(column_confidence_values)))
            ** number_of_disagreeing_attributes
        )
    ...
    # exact
        ...
        score_counts = {
            x: column_confidence_values.count(x) for x in column_confidence_values
        }
        prod_list = []
        for score, count in score_counts.items():
            prod_list.append((1 - score) ** count)
...
def disagreement_confidence(
    disagreements, nonnull_values, p=0.4
):  # , p=0.5, PA=1, PB=10):
    ...
    if len(disagreements) == 1 and disagreements[0] == "SC":
        ...
        disagreements[0] == "FW"
        or disagreements[0] == "BW"
        or disagreements[0] == "CC"
        or disagreements[0] == "SS"
    ):
    ...
    if singletable == False:
        with codecs.open(filepath, "rU", encoding=encoding) as f:
            csv_reader = csv.reader(
                f, delimiter=delimiter, skipinitialspace=True, quotechar='"'
            )
        ...
        if len(file_blank_rowindex) > 1:
            ...
            for index, i in enumerate(rev_blanklines):
                if (
                    index + 1 < len(rev_blanklines)
                    and i != rev_blanklines[index + 1] + 1
                ):
                    lastconsecutive = index
                    break
                elif (
                    index + 1 == len(rev_blanklines)
                    and i + 1 == rev_blanklines[index - 1]
                ):
                    lastconsecutive = index
                    break

        if (
            len(file_blank_rowindex) > 1
            and num_rows == file_blank_rowindex[-1] + 1
            and lastconsecutive > 0
        ):
        ...
        while more_tables_in_file and offset < num_rows:
            ...
            table_info = split_metadata_data(
                csv_tuples, offset, sample_lines_limit, delimiter
            )
            ...
            if table_info["metadatastart"] != datastart:
                ...
                pre_metafiles[relation_counter]["file_index"] = table_info[
                    "metadatastart"
                ]
            ...
            if sum(i > datastart for i in file_blank_rowindex) > 0:
                ...
                post_meta = csv_tuples[
                    datastart + len(datatuples) + 1 : candidate_dataend
                ]
                ...
                (
                    ...
                    outlier_aware_summaries,
                    outliers,
                ) = discover_header_and_datastart(df)
                # LOG
                ...
                if relation_counter in pre_metafiles.keys():
                    premeta["from_line_idx"] = pre_metafiles[relation_counter][
                        "file_index"
                    ]
                ...
                premeta["premeta_text"] = csv_tuples[
                    premeta["from_line_idx"] : premeta["to_line_idx"]
                ]  # premeta_text

                header["from_line_idx"] = offset + headerindex[0]
                header["to_line_idx"] = offset + headerindex[1]
                header["header_text"] = df[
                    headerindex[0] : headerindex[1]
                ].values.tolist()  # csv_tuples[header["from_line_idx"]:header["to_line_idx"]]
                            ...
                            try:
                                for y in dataframe.iloc[:, 1].values:
                                    if y == None or y.strip() == "":
                                        data[
                                            "first_two_columns_have_emptyvalues"
                                        ] = True
                                        break
                            except:
                                data["first_two_columns_have_emptyvalues"] = False
                ...
                data["five_lines"] = df[
                    data["from_line_idx"] : min(
                        (df.shape[0] - data["from_line_idx"] - 1), 5
                    )
                ].to_csv(sep=delimiter, index=False, header=False)

                postmeta["from_line_idx"] = (
                    offset + datastart_index + dataframe.shape[0]
                )
                postmeta["to_line_idx"] = candidate_dataend
                postmeta["postmeta_text"] = csv_tuples[
                    postmeta["from_line_idx"] : postmeta["to_line_idx"]
                ]  # post_meta
                ...
                if len(first_disagreeing_row) > 0:
                    first_disagreeing_row["line_idx"] = (
                        offset + first_disagreeing_row["index"]
                    )
                ...
                if singletable and metadataseen == False:
                    ...
                    dataend = discover_dataend_idx(candidate_datatuples)
                    if headerindex[0] != headerindex[1]:
                        dataheaders[relation_counter] = df[
                            headerindex[0] : headerindex[1]
                        ].values.tolist()
                    if headerindex[1] == 0:
                        premetaend = datastart_index
        ...
        header["header_text"] = df[
            headerindex[0] : headerindex[1]
        ].values.tolist()  # batch[header["from_line_idx"]:header["to_line_idx"]]
        header["header_rule"] = header_detection_rule
        if headerindex[0] != headerindex[1]:
            header["combo"] = combo_row(df[headerindex[0] : headerindex[1]])
            header["attribute_patterns"] = row_patterns(header["combo"])
        ...
        data["num_columns"] = df.shape[1]
        data["five_lines"] = df[
            data["from_line_idx"] : min((df.shape[0] - data["from_line_idx"] - 1), 5)
        ].to_csv(sep=delimiter, index=False, header=False)
