...
import ast
import pandas as pd
from pytheas.pat_utilities import (
    generate_pattern_symbols_and_case,
...
def recreate_data_block(
    ...
    endpoint_con = connect(
        dbname=endpoint_dbname,
        user="christina",
        host="localhost",
        password="marathi",
        port=5532,
    ...
    endpoint_cur.execute(
        """SELECT * FROM """ + sql_table_name + """ limit """ + str(row_limit)
    )
    tuples = []
    ...
    if has_header:
        cur.execute(
            """SELECT header_text from dataheader where datatable = %s""",
            (datatable_key,),
        )
        res = cur.fetchone()
        if res:
    ...
    if has_premeta:
        cur.execute(
            """SELECT premeta_text from pre_metadata where datatable = %s""",
            (datatable_key,),
        )
        res = cur.fetchone()
        if res:
...
def test_discover_numeric_tokens():
    NS = [
        "  23456000and23456000. or23456000.000  00023456000 s000023456000.  000023456000.000 ",
    ...
    try:
        if pd.isna(input_value) == False and input_value != None:
            ...
            for tok_idx, numeric_tok in enumerate(numeric_tokens):
                occurence_index_pairs = [
                    (m.start(), m.end()) for m in re.finditer(numeric_tok, input_value)
                ]
            ...
            while tok_idx < len(numeric_tokens):
                ...
                if e_idx < len(input_value) and input_value[e_idx].strip() == "":
                    ...
                    while (
                        tok_idx + 1 < len(token_to_stridx)
                        and token_to_stridx[tok_idx + 1][0]
                        == token_to_stridx[tok_idx][1] + 1
                        and input_value[token_to_stridx[tok_idx][1]].strip() == ""
                    ):
                        if (
                            first_flag == True
                            and re.match(r"^[1-9]\d{1,2}$", numeric_tok)
                        ) or (
                            first_flag == False
                            and re.match(r"^\d{3}$", numeric_tokens[tok_idx + 1])
                        ):
                            numeric_tok = (
                                numeric_tok
                                + input_value[
                                    token_to_stridx[tok_idx][1] : token_to_stridx[
                                        tok_idx + 1
                                    ][1]
                                ]
                            )
...
def test_discover_range_tokens():
    ...
    for value in test_strings:
        ...
        range_tokens, numeric_tokens, remaining_tokens = discover_range_tokens(
            value, numeric_tokens_new
        )
        input(
            """
- - - > range_tokens = """
...
def discover_range_tokens(value, numeric_tokens):
    ...
    if value != None:
        try:
            ...
            range_phrases = [
                "(?:(mois depuis )\s*[a-zA-Z]*\s*(REGEX_TKN))",
            ...
            range_phrases_2 = [
                r"(?:(^|\s+)(REGEX_TKN)([-–/])(REGEX_TKN)(\s+|$))",  # date exceptions
            ...
            for range_phrase in range_phrases_2:
                ...
                for numeric_token1, numeric_token2 in numeric_token_pairs:
                    if (
                        numeric_token1,
                        numeric_token2,
                    ) not in available_numeric_token_pairs:
                        continue
                    ...
                    if phrase_start_idx != -1:
                        ...
                        if (
                            range_phrase
                            == "(?:(^|\s+)(REGEX_TKN)([-–/])(REGEX_TKN)(\s+|$))"
                        ):
                            if len(numeric_token1) == 4 and len(numeric_token2) == 2:

                                if (
                                    evaluate(numeric_token2)
                                    != evaluate(numeric_token1[-2:]) + 1
                                ):
                                    available_numeric_token_pairs.remove(
                                        (numeric_token1, numeric_token2)
                                    )
                                    snipped_value = re.sub(
                                        "(?i)" + phrase_found,
                                        " ",
                                        snipped_value,
                                        flags=re.I,
                            ...
                            if (
                                phrase_start_idx > 0
                                and value[phrase_start_idx - 1] == delimiter
                            ):
                                available_numeric_token_pairs.remove(
                                    (numeric_token1, numeric_token2)
                                )
                                ...
                                continue
                            if (
                                phrase_end_idx < len(value)
                                and value[phrase_end_idx] == delimiter
                            ):
                                available_numeric_token_pairs.remove(
                                    (numeric_token1, numeric_token2)
                                )
                        ...
                        snipped_value = re.sub(
                            "(?i)" + phrase_found, " ", snipped_value, flags=re.I
                        )
