...
def save_table_attributes(datatable_key, attributes):
    ...
    datacolumn_table = Table(
        "datacolumn", metadata, autoload=True, autoload_with=opendata_engine
    )
...
def label_attributes_in_file(task):
    with opendata_engine.begin() as conn:
        conn.execute(
            """DELETE FROM label_attributes WHERE datafile = %s""", (task.datafile_key)
        )
    try:
        ...
        tables = sorted(
            task.annotations["tables"], key=lambda table: table["table_counter"]
        )

        for table in tables:

            discovered_body_dataframe = file_dataframe.loc[
                table["data_start"] : table["data_end"]
            ]

            discovered_body_dataframe = discovered_body_dataframe.convert_dtypes(
                infer_objects=True,
                convert_string=True,
            ...
            for c in discovered_body_dataframe.columns:
                discovered_body_dataframe[c] = pd.to_numeric(
                    discovered_body_dataframe[c], errors="ignore"
                )
            discovered_header_dataframe = pd.DataFrame()

            if header_upper_bound != table["data_start"]:
                candidate_header_dataframe = file_dataframe.loc[
                    header_upper_bound : table["data_start"] - 1
                ]
                discovered_header_idxs, _ = predict_header_indexes(
                    pd.concat(
                        [candidate_header_dataframe, discovered_body_dataframe],
                        ignore_index=True,
                    ),
                    len(candidate_header_dataframe),
                    table["table_counter"],
                )
                discovered_header_idxs = [
                    i + header_upper_bound for i in discovered_header_idxs
                ]
                if len(discovered_header_idxs) > 0:
                    discovered_header_dataframe = file_dataframe.loc[
                        discovered_header_idxs
                    ]
            ...
            try:
                table_dataframe = pd.concat(
                    [discovered_header_dataframe, discovered_body_dataframe]
                )
            except:
            ...
            for attribute_idx, csv_idx in enumerate(table_dataframe.columns):
                ...
                table_attributes[attribute_idx] = {
                    "csv_idx": csv_idx,
                    "attribute_name": None
                    if pd.isnull(attribute_names[attribute_idx])
                    else attribute_names[attribute_idx],
                    "merged_name": merged_name,
                    "pd_dtype": pd.api.types.infer_dtype(
                        discovered_body_dataframe[csv_idx], skipna=True
                    ),
                }
...
def task_generator(files):
    for datafile in files:
        ...
        with opendata_engine.connect() as conn:
            result = conn.execute(
                """SELECT datatable_key, table_index, from_index, to_index
                                        FROM datatable
                                        WHERE datafile = %s
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-i", "--input_portals", nargs="*", default=["open.canada.ca_data"]
    )
    parser.add_argument(
        "-d", "--crawl_directory", default="/home/christina/OPEN_DATA_CRAWL_2018"
    )
    ...
    if not opendata_engine.dialect.has_table(
        opendata_engine, "label_attributes"
    ...
    for portal in args.input_portals:
        with opendata_engine.connect() as conn:
            result = conn.execute(
                """SELECT count(1) as count 
                                     ...
                                     WHERE endpoint_dbname = %s
                                            AND datafile_key in 
                                                (SELECT datafile from datatable)""",
                (portal,),
            )
            ...
            files = conn.execute(
                """SELECT datafile_key, path, encoding, delimiter
                            FROM datafile 
                            WHERE endpoint_dbname = %s
            ...
            with Pool(processes=args.NPROC) as pool:
                with tqdm(total=NINPUTS) as pbar:
                    for r in pool.imap_unordered(
                        label_attributes_in_file, task_generator(files)
                    ):
                        pbar.update(1)
