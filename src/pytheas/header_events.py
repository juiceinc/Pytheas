...
def arithmetic_sequence_adjacent(row_values, step_count_k=2):
    ...
    step_increment, step_count = utilities.discover_incremental_values(
...
def arithmetic_sequence_non_adjacent(row_values, step_count_k=2):
    ...
    (
        step_increment,
        step_count,
    ) = utilities.discover_incremental_values_at_least_one_nonadjacent(
        row_values, sample_symbols
    )
    # print('step_increment='+str(step_increment))
    # print('step_count='+str(step_count))
...
def consistently_upper_case(row_values):
    fired = True
    values = [
        str(value).strip() if str(value).lower() not in ["nan", "none"] else ""
        for value in row_values
    ]
    for value in values:
...
def integer_sequence_adjacent(row_values):
    ...
    if len(row_values) >= 2:
        ...
        for i in range(len(numeric_values) - 1):
            if (
                numeric_values[i] != None
                and numeric_values[i + 1] != None
                ...
                if sequence_found == True:
                    if (
                        i > 0
                        and numeric_values[i - 1] + 1 == numeric_values[i]
                        and numeric_values[i] != None
                    ):
                        sequential_values += [numeric_values[i]]
        ...
        if sequence_found == True:
            if (
                numeric_values[-2] + 1 == numeric_values[-1]
                and numeric_values[-1] != None
            ):
    ...
    for value in values:
        ...
        for char in value:
            if (
                ...
                or char == ","
                or char == " "
                or (len(value) > 1 and value.startswith("-") and value[1].isdigit())
            ):
                continue
            else:
                is_number = False
...
def header_row_with_aggregation_tokens(row_values, arithmetic_sequence_fired):
    ...
    if aggregation_on_row_wo_numeric(row_values):
        header_row_with_aggregation_tokens_rules_fired.append(
...
def consistently_slug_case(row_values):
    ...
    for value in row_values:
        if " " in unidecode(value).strip():
            ...
            break
        if (
            "-" in unidecode(value).strip()
            and string_utils.is_slug(unidecode(value).strip().lower()) == True
        ):
            slug_case_seen = True
        else:
...
def range_pairs_on_row(row_values):
    ...
    for value_idx, value in enumerate(row_values):
        ...
        range_tokens, numeric_tokens, remaining_tokens = nb_util.discover_range_tokens(
...
def repeating_values_on_row(row_values):
    ...
    if (
        condition_failed == False
        and repeating_value_seen
        and max(repeating_lengths) > 0
        and max(repeating_seen_list) > 1
        and len(set(repeating_seen_list)) == 1
    ):
        event_occurred = True
        # print("\n---REPEATING VALUES")
...
def partially_repeating_values_on_row(row_values):
    ...
    if len(repeating_seen_list) > 0:
        ...
        if (
            ...
            and max(repeating_lengths) > 0
            and max_repeats >= 2
            and repeating_seen_list.count(max_repeats) >= 2
        ):
            event_occurred = True
    else:
        max_repeats = 0
