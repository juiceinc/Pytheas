...
def annotateCSV(filepath, max_lines):
    ...
    ) = file_utilities.sample_file(filepath, max_lines)
        ...
        if has_tables == "n":
            ...
            while int(context_end) + 1 <= dataframe.index[-1]:
                ...
                if multiple_tables == "n":
                    ...
                    context_start = input(
                        "\n\nIndicate TOP_BOUNDARY of table context starting at row index "
                        + str(int(context_end) + 1)
                        + ": \n"
                    )
                    context_end = input(
                        "\nIndicate BOTTOM_BOUNDARY of table context after row index "
                        + str(context_start)
                        + ": \n"
                    )
                    print(
                        "Table context is indicated between row index "
                        + str(context_start)
                        + " and row index "
                        + str(context_end)
                        + ". \n"
                ...
                data_start = input(
                    "\n\nIndicate index of START of TABLE BODY section within rows "
                    + str(context_start)
                    + " and "
                    + str(context_end)
                    + ":\n"
                ...
                data_end = input(
                    "\nIndicate index of END of TABLE BODY section within rows "
                    + str(data_start)
                    + " and "
                    + str(context_end)
                    + ":\n"
                ...
                if int(data_start) > int(context_start):
                    ...
                    user_input = input(
                        "\n\nIndicate indexes of HEADER for table before row index "
                        + str(data_start)
                        + ": \n"
                    )
                    if len(user_input) > 0:
                ...
                if user_input == "y":
                    user_input = input(
                        "\nIndicate indexes of SUB_HEADERS for table between row index "
                        + str(data_start)
                        + " and row index "
                        + str(data_end)
                        + ": \n"
                ...
                data_indexes = sorted(
                    list(
                        set(range(int(data_start), int(data_end) + 1))
                        - set(subheader_indexes)
                        - set(blanklines)
                    )
                )
                print(data_indexes)
                ...
                table_context["not_data"] = sorted(
                    list(
                        set(range(int(context_start), int(context_end) + 1))
                        - set(data_indexes)
                    )
                )
                footnotes = []
                if int(data_end) < int(context_end):
                    footnotes = sorted(
                        list(
                            set(range(int(data_end) + 1, int(context_end) + 1))
                            - set(blanklines)
                        )
                    )
...
def annotateCSVfolder(csv_dir, annotations_dir, max_lines):
    for fileindex in sorted(
        ...
        annotation_path = Path(os.path.join(annotations_dir, filename)).with_suffix(
            ".json"
        )
        print(f"annotation_path={annotation_path}")
        if not os.path.exists(annotation_path):
            file_annotations = annotateCSV(filepath, max_lines)
...
if __name__ == "__main__":
    ...
    file_parser.add_argument(
        "-f", "--filepath", help="path to CSV file for user annotation"
    )
    file_parser.add_argument("-o", "--output_file", default=None)
    ...
    if args.context == "file":
        file_annotations = annotateCSV(args.filepath, args.max_lines)
        if args.output_file is not None:
            with open(args.output_file, "w") as outfile:
                json.dump(file_annotations, outfile)
    elif args.context == "folder":
        annotateCSVfolder(
            args.csv_directory, args.annotations_directory, args.max_lines
        )
