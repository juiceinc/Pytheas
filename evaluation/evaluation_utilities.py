...
def evaluate_relation_extraction(annotated_tables, discovered_tables):
    ...
    for dt in discovered_tables.values():
        ...
        for at in annotated_tables:
            if (
                dt["data_start"] == at["data_start"]
                and dt["data_end"] == at["data_end"]
            ):
        ...
        table_confidences_tuples.append(
            [
                table_counter,
                top_confidence,
                bottom_confidence,
    ...
    table_confidences = pd.DataFrame(
        table_confidences_tuples,
        columns=[
            "table_counter",
            "top_confidence",
...
def avg_and_confidence(frame, label, num_folds):
    average_performance = (frame.mean() * 100).reindex(
        ["precision", "recall", "fmeasure"]
    )
    performance_interval = (frame.sem() * 100).reindex(
        ["precision", "recall", "fmeasure"]
    )
...
def average_performance(pkl_filepath):
    ...
    if "line" in results[1].keys():
        avg, interval = avg_and_confidence(
            pd.concat([results[fold_id]["line"] for fold_id in results.keys()]).groupby(
                level=0
            ),
            "average_line_performance",
            num_folds,
        )
    ...
    if "boundary_performance" in results[1].keys():
        avg, interval = avg_and_confidence(
            pd.concat(
                [results[fold_id]["boundary_performance"] for fold_id in results.keys()]
            ).groupby(level=0),
            "average_boundary_performance",
            num_folds,
        )
    ...
    if "cell" in results[1].keys():
        avg, interval = avg_and_confidence(
            pd.concat([results[fold_id]["cell"] for fold_id in results.keys()]).groupby(
                level=0
            ),
            "average_cell_performance",
            num_folds,
        )
    ...
    if "first_table_line" in results[1].keys():
        avg_and_confidence(
            pd.concat(
                [results[fold_id]["first_table_line"] for fold_id in results.keys()]
            ).groupby(level=0),
            "average_first_table_line_performance",
            num_folds,
        )
    ...
    if "first_table_boundary_performance" in results[1].keys():
        avg_and_confidence(
            pd.concat(
                [
                    results[fold_id]["first_table_boundary_performance"]
                    for fold_id in results.keys()
                ]
            ).groupby(level=0),
            "average_first_table_boundary_performance",
            num_folds,
        )
    ...
    if "table" in results[1].keys():
        avg, interval = avg_and_confidence(
            pd.concat(
                [results[fold_id]["table"] for fold_id in results.keys()]
            ).groupby(level=0),
            "average_relation_performance",
            num_folds,
        )
    ...
    if "file_jaccard" in results[1].keys():
        jaccard_cell_accuracies = [
            results[fold_id]["file_jaccard"] for fold_id in results.keys()
        ]
        avg_jaccard_cell_accuracy = sum(jaccard_cell_accuracies) / len(
            jaccard_cell_accuracies
        )
        print(
            f"avg jaccard data section cell accuracy (over {num_folds} folds)={avg_jaccard_cell_accuracy}"
        )
...
def predict_performance(labels, y_test, y_pred):
    ...
    for label in labels:
        ...
        performance.loc["precision", label] = precision(
            true_positive, predicted_positive
        )
        performance.loc["recall", label] = recall(true_positive, real_positive)
        performance.loc["fmeasure", label] = fmeasure(
            performance.loc["precision", label], performance.loc["recall", label]
        )
...
def assign_class(
    ...
    for table_counter in iter(discovered_tables):
        ...
        if table != None:
            ...
            data[table_counter] = sorted(
                list(set(data_section) - set(blank_lines) - set(subheader_scope.keys()))
            )
    ...
    if "tables" in annotations.keys():
        for table in annotations["tables"]:
            ...
            gt_data[gt_table_counter] = sorted(
                list(
                    set(list(range(table["data_start"], table["data_end"] + 1)))
                    - set(table["subheaders"])
                )
            )
