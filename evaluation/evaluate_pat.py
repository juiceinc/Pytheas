...
def eval_pat_line_kfold(
    ...
    cur.execute(
        f"""CREATE TABLE "{k_folds}fold_cross_validation_pat_model"(
        fold_id integer,
        fuzzy_rules json
    ) """
    )
    ...
    cur.execute(
        f"""CREATE TABLE "{k_folds}fold_cross_validation_pat_cells"(
        fold_id integer,
        measure text
    ) """
    )
    ...
    for label in labels:
        cur.execute(
            f"""ALTER TABLE "{k_folds}fold_cross_validation_pat_cells" add column {label} real"""
        )
    con.commit()
    cur.execute("DROP TABLE IF EXISTS pat_cell_predictions")
    con.commit()
    cur.execute(
        """CREATE TABLE pat_cell_predictions(
                    fold_id integer,
    ...
    cur.execute(
        """CREATE TABLE pat_line_predictions(
                    fold_id integer,
    ...
    cur.execute(
        f"""CREATE TABLE "{k_folds}fold_cross_validation_pat_lines"(
        fold_id integer,
        measure text
    ) """
    )
    ...
    for label in labels:
        cur.execute(
            f"""ALTER TABLE "{k_folds}fold_cross_validation_pat_lines" add column {label} real"""
        )
    con.commit()
    ...
    folds = dict()
    cur.execute(
        f"""SELECT fold_id, training, validation  FROM "{k_folds}fold_cross_validation" """
    )
    ...
    for fold_id in folds.keys():
        ...
        pat_line_classification_DATA = list(
            line_prediction.itertuples(index=False, name=None)
        )
        pat_cell_classification_DATA = list(
            cell_prediction.itertuples(index=False, name=None)
        )
        con = connect(
            dbname=db_cred.database,
            user=db_cred.user,
        ...
        execute_values(
            cur,
            """INSERT INTO pat_line_predictions 
        ...
        execute_values(
            cur,
            """INSERT INTO pat_cell_predictions 
        ...
        #### PERFORMANCE ###
        line_performance = evaluation_utilities.predict_performance(
            labels,
            line_prediction["annotated_label"],
            line_prediction["predicted_label"],
        )
        print(f"\n\nline_performance=\n\n{line_performance}")
        results[fold_id]["line"] = line_performance
        evaluation_utilities.save_performance(
            db_cred,
            line_performance,
            f'"{k_folds}fold_cross_validation_pat_lines"',
            fold_id,
        )

        boundary_performance = evaluation_utilities.predict_performance(
            ["DATA_START"],
            line_prediction["annotated_fdl"],
            line_prediction["predicted_fdl"],
        )

        data_end_performance = evaluation_utilities.predict_performance(
            ["DATA_END"],
            line_prediction["annotated_ldl"],
            line_prediction["predicted_ldl"],
        )

        boundary_performance = boundary_performance.join(
            data_end_performance, how="outer"
        )
        evaluation_utilities.save_performance(
            db_cred,
            boundary_performance,
            f'"{k_folds}fold_cross_validation_pat_boundary"',
            fold_id,
        )
        print(f"\n\nboundary_performance=\n\n{boundary_performance}")
        results[fold_id]["boundary_performance"] = boundary_performance

        first_table_line_performance = evaluation_utilities.predict_performance(
            labels,
        ...
        evaluation_utilities.save_performance(
            db_cred,
            first_table_line_performance,
            f'"{k_folds}fold_cross_validation_pat_line_first"',
            fold_id,
        )

        first_table_boundary_performance = evaluation_utilities.predict_performance(
            ["DATA_START"],
        ...
        first_table_boundary_performance = first_table_boundary_performance.join(
            data_end_performance, how="outer"
        )
        results[fold_id][
        ...
        evaluation_utilities.save_performance(
            db_cred,
            first_table_boundary_performance,
            f'"{k_folds}fold_cross_validation_pat_boundary_first"',
            fold_id,
        )
        ...
        cell_performance = evaluation_utilities.predict_performance(
            labels,
        ...
        evaluation_utilities.save_performance(
            db_cred,
            cell_performance,
            f'"{k_folds}fold_cross_validation_pat_cell"',
            fold_id,
        )

        table_confusion_matrices = [
            worker_output[3] for worker_output in processed_files
        ]

        table_confusion_matrix = [
            [
                t.real_positive,
                t.true_positive_table,
                t.true_positive_data,
                t.predicted_positive_table,
                t.predicted_positive_data,
            ]
            for t in table_confusion_matrices
        ]
        (
            real_positive,
            true_positive_table,
        ...
        ) = map(sum, zip(*table_confusion_matrix))

        section_performance = pd.DataFrame(columns=["Data", "Data_Header"]).astype(
            np.float
        )
        section_performance.loc["precision", "Data"] = evaluation_utilities.recall(
            true_positive_data, predicted_positive_data
        )
        section_performance.loc["recall", "Data"] = evaluation_utilities.recall(
            true_positive_data, real_positive
        )
        section_performance.loc["fmeasure", "Data"] = evaluation_utilities.fmeasure(
            section_performance.loc["precision", "Data"],
            section_performance.loc["recall", "Data"],
        )
        section_performance.loc[
            "precision", "Data_Header"
        ] = evaluation_utilities.recall(true_positive_table, predicted_positive_table)
        section_performance.loc["recall", "Data_Header"] = evaluation_utilities.recall(
            true_positive_table, real_positive
        )
        section_performance.loc[
            "fmeasure", "Data_Header"
        ] = evaluation_utilities.fmeasure(
            section_performance.loc["precision", "Data_Header"],
            section_performance.loc["recall", "Data_Header"],
        )
        evaluation_utilities.save_performance(
            db_cred,
            section_performance,
            f'"{k_folds}fold_cross_validation_pat_relation"',
            fold_id,
        )
        ...
        file_data_jaccard = evaluation_utilities.jaccard_similarity_coefficient(
            ["DATA", "SUBHEADER"],
            cell_prediction["annotated_label"],
            cell_prediction["predicted_label"],
        )
        ...
        table_confidences["avg_confidence"] = table_confidences[
            ["top_confidence", "bottom_confidence"]
        ].mean(axis=1)
        ...
        table_confidences.head(0).to_sql(
            "pat_table_confidences", engine, if_exists="append", index=False
        )
    ...
    if "tables" in annotations:
        ...
        for table in annotated_tables:
            if "data_start" in table.keys():
                ...
                labels = [
                    (row, col)
                    for row in range(datastart, dataend + 1)
                    for col in df.columns
                ]
                for row, col in random.sample(
                    labels, int(round(inject_percent / 100 * len(labels)))
                ):
                    if df.loc[row, col] not in [None, np.nan]:
                        noise = generate_noise()
                        df.loc[row, col] = (
                            str(df.loc[row, col]) + noise
                            if random.choice([True, False])
                            else noise + str(df.loc[row, col])
                        )
    ...
    if "tables" in annotations:
        ...
        for table in annotated_tables:
            if "data_start" in table.keys():
                ...
                for row, col in random.sample(
                    labels, int(round(inject_percent_null / 100 * len(labels)))
                ):
...
def generate_process_csv_tasks(
    db_cred,
    top_level_dir,
    pat_classifier,
    fold_id,
    ...
    cur = con.cursor()
    cur.execute(
        """SELECT crawl_datafile_key, groundtruth_key, annotations, original_path, failure
...
def process_csv_worker(task):
    ...
    if inject_percent_null != None:
        file_dataframe_trimmed = inject_nulls(
            file_dataframe_trimmed, annotations, inject_percent_null
        )
    if inject_percent_outlier != None:
        file_dataframe_trimmed = inject_noise(
            file_dataframe_trimmed, annotations, inject_percent_outlier
        )

    discovered_tables = pat_classifier.extract_tables(
        file_dataframe_trimmed, blank_lines
    )
    line_predictions, cell_predictions = assign_class(
        db_cred,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-d",
        "--database",
        default="ground_truth_2k_canada",
        help="database for experimentation with ground truth",
    )
    parser.add_argument(
        "-u", "--user", default="christina", help="user for the database connection"
    )
    parser.add_argument(
        "-p", "--port", default=5532, help="port that postgresql database listens to"
    )
    ...
    parser.add_argument(
        "-t",
        "--top_level_dir",
        default="/home/christina/OPEN_DATA_CRAWL_2018",
    ...
    db_cred.password = getpass(
        prompt=f"Please enter password for user {db_cred.user} on database {db_cred.database}:"
    )
    max_lines = 100

    pat_classifier = pat.PYTHEAS()
    average_results = eval_pat_line_kfold(
        pat_classifier,
        k_folds,
        db_cred,
        top_level_dir,
